...
class Job {
    ...
    /**
     * Get all active jobs with optional filtering (keyword, category, job_type, location, status)
     */
    public function getAllJobs($filters = []) {
        $sql = "
            SELECT j.*, c.company_name, c.logo, c.industry, cat.name AS category_name, cat.icon AS category_icon
            FROM jobs j
        ...
        if (!empty($filters['keyword'])) {
            $sql .= " AND (j.title LIKE :keyword OR j.description LIKE :keyword OR c.company_name LIKE :keyword)";
    ...
    /**
     * Get job by ID with detailed company and category info
     */
    public function getJobById($id) {
        $this->db->query("
            SELECT j.*, 
                   c.company_name, c.website, c.industry, c.company_size, c.description AS company_description, c.logo, c.address AS company_address, c.user_id AS employer_user_id,
    ...
    public function createJob($data) {
        $this->db->query("
            INSERT INTO jobs (company_id, category_id, title, description, requirements, location, salary, job_type, status, expiry_date)
            VALUES (:company_id, :category_id, :title, :description, :requirements, :location, :salary, :job_type, :status, :expiry_date)
        ");
        ...
        $this->db->bind(':requirements', trim($data['requirements']));
        $this->db->bind(':location', trim($data['location']));
        $this->db->bind(':salary', trim($data['salary']));
    ...
    public function toggleJobStatus($id, $status) {
        $this->db->query("UPDATE jobs SET status = :status WHERE id = :id");
        $this->db->bind(':id', $id);
