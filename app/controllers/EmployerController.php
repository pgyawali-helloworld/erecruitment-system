...
class EmployerController extends Controller {
    ...
    private function getEmployerCompany() {
        $employer = $this->userModel->getUserDetails(Session::get('user_id'), 'employer');
        if (!$employer || empty($employer->company_id)) {
            Session::setFlash('error', 'Company profile is incomplete. Please contact support.');
            return null;
        }
        return $employer;
    }
    ...
    public function dashboard() {
        ...
        if ($employer) {
            $jobs = $this->jobModel->getJobsByCompanyId($employer->company_id);
            $applications = $this->applicationModel->getEmployerApplications($employer->company_id);
        }
    ...
    public function createJob() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // if (empty($job_type)) $errors['job_type'] = 'Job type is required.';
            if (empty($location)) $errors['location'] = 'Job location is required.';
...
if ($salary === '') {
    ...
    $errors['expiry_date'] = 'Expiry date cannot be in the past.';
            ...
            if (empty($errors)) {
                ...
                if ($this->jobModel->createJob($jobData)) {
                    Session::setFlash('success', 'Job vacancy posted successfully!', 'alert-success');
                    $this->redirect('employer/jobs');
                } else {
                    Session::setFlash('error', 'Something went wrong. Failed to create job.');
                }
            }
    ...
    public function showEditJob($id) {
        ...
        if (!$job || $job->company_id != $employer->company_id) {
            Session::setFlash('error', 'Job vacancy not found or access unauthorized.');
            $this->redirect('employer/jobs');
        }
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (empty($errors)) {
                ...
                if ($this->jobModel->updateJob($id, $jobData)) {
                    Session::setFlash('success', 'Job vacancy updated successfully!', 'alert-success');
                    $this->redirect('employer/jobs');
                } else {
                    Session::setFlash('error', 'Failed to update job vacancy.');
                }
            }
            ...
            $data = [
                ...
                'job' => (object) array_merge((array)$job, $_POST),
                'categories' => $categories,
                'errors' => $errors,
                'activeTab' => 'jobs'
            ];
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($job && $job->company_id == $employer->company_id) {
                if ($this->jobModel->deleteJob($id)) {
                    Session::setFlash('success', 'Job vacancy deleted successfully.', 'alert-success');
                } else {
                    Session::setFlash('error', 'Failed to delete job vacancy.');
                }
            } else {
                Session::setFlash('error', 'Unauthorized operation or job not found.');
            }
        }
        $this->redirect('employer/jobs');
    }
    ...
    /**
     * Update Candidate Application Status (pending, under_review, accepted, rejected)
     */
    public function updateApplicationStatus($id) {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $status = isset($_POST['status']) ? trim($_POST['status']) : 'pending';
            $validStatuses = ['pending', 'under_review', 'accepted', 'rejected'];

            if (in_array($status, $validStatuses)) {
                if ($this->applicationModel->updateStatus($id, $status)) {
                    Session::setFlash('success', 'Application status updated to ' . str_replace('_', ' ', $status) . '.', 'alert-success');
                } else {
                    Session::setFlash('error', 'Failed to update application status.');
                }
            }
        }
